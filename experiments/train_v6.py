"""
TailFlow v6: S4D + time features (hour-of-day, day-of-week).

Critical for solar (night zeros) and traffic (rush hours).
Also trains for more epochs to push electricity past TSFlow.

Usage:
    python experiments/train_v6.py electricity_nips --epochs 1500
    python experiments/train_v6.py solar_nips --epochs 1500
"""
...
from meanflow_ts.model_v6 import (
    S4DMeanFlowNetV6, v6_meanflow_loss, V6Forecaster,
...
def evaluate_model(net_ema, dataset, transformation, cfg, device,
    ...
    forecaster = V6Forecaster(
        net_ema, ctx_len, pred_len, num_samples=num_samples, freq=freq,
    ).to(device)

    # Need to pass time_feat through predictor
...
def main():
    ...
    n_time_features = len(time_features_from_frequency_str(freq))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    torch.manual_seed(42)
    np.random.seed(42)

    outdir = os.path.join(os.path.dirname(__file__), '..', 'results_v6', name)
    os.makedirs(outdir, exist_ok=True)

    logger.info(f"{'='*60}")
    logger.info(f"TailFlow v6: {name}")
    logger.info(f"ctx={ctx_len} pred={pred_len} freq={freq}")
    logger.info(f"n_lags={n_lags} n_time_features={n_time_features}")
    logger.info(f"d_model={args.d_model} n_blocks={args.n_blocks}")
    logger.info(f"{'='*60}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_loader:
            ...
            past_tf = batch["past_time_feat"].to(device).transpose(1, 2)  # (B, n_tf, past_len)
            future_tf = batch["future_time_feat"].to(device).transpose(1, 2)  # (B, n_tf, pred_len)
            ctx = past[:, -ctx_len:]
            ...
            tf_ctx = past_tf[:, :, -ctx_len:]
            ctx_channels = torch.cat([lags_normed, tf_ctx], dim=1)

            loss = v6_meanflow_loss(net, future_normed, ctx_channels, future_tf)
            optimizer.zero_grad()
            loss.backward()
        ...
        if (epoch + 1) % 20 == 0 or epoch == 0:
            logger.info(f"Ep {epoch+1:>4}/{args.epochs} | Loss: {avg:.4f} | "
        ...
        if (epoch + 1) % args.eval_every == 0 or (epoch + 1) == args.epochs:
            metrics = evaluate_model(net_ema, dataset, transformation, cfg, device,
                                      num_samples=16, label=f"ep{epoch+1}", freq=freq)
            crps = metrics["mean_wQuantileLoss"]
            if crps < best_crps:
                best_crps = crps
                torch.save({
            ...
            beat = " *** BEATS TSFLOW! ***" if isinstance(tsflow, float) and crps < tsflow else ""
            logger.info(f"  CRPS={crps:.6f} | TSFlow={tsflow} | Best={best_crps:.6f}{tag}{beat}")

    # Final eval
    logger.info("\nFinal evaluation (100 samples)...")
    m = evaluate_model(net_ema, dataset, transformation, cfg, device,
                        num_samples=100, label="final", freq=freq)
    tsflow = TSFLOW_CRPS.get(name, "?")
    logger.info(f"\n{'='*60}")
    logger.info(f"FINAL: {name}")
    logger.info(f"  TSFlow (32 NFE): {tsflow}")
    logger.info(f"  TailFlow v6 (1 NFE): Best={best_crps:.6f}, Final(100)={m['mean_wQuantileLoss']:.6f}")
    logger.info(f"{'='*60}")

    with open(os.path.join(outdir, 'results.json'), 'w') as f:
        json.dump({
            'dataset': name, 'best_crps': best_crps,
            'final_crps': m['mean_wQuantileLoss'], 'params': param_count,
        }, f, indent=2)
