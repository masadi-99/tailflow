"""
TailFlow v5: Bidirectional S4D + Cross-Attention + Observation Mask.

Goal: Beat TSFlow on ALL datasets.
"""
...
from meanflow_ts.model_v5 import (
    S4DMeanFlowNetV5, v5_meanflow_loss, V5Forecaster,
)
...
def main():
    ...
    outdir = os.path.join(os.path.dirname(__file__), '..', 'results_v5', name)
    os.makedirs(outdir, exist_ok=True)

    logger.info(f"{'='*60}")
    logger.info(f"TailFlow v5: {name}")
    ...
    results = {'dataset': name, 'config': cfg}
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_loader:
            ...
            # Observation mask: 1 where context is observed (non-zero raw value)
            obs_mask = (ctx.abs() > 1e-6).float().unsqueeze(1)

            loss = v5_meanflow_loss(net, future_normed, lags_normed, obs_mask)
            optimizer.zero_grad()
            loss.backward()
    ...
    results['final'] = {'crps': m["mean_wQuantileLoss"], 'nd': m["ND"], 'nrmse': m["NRMSE"]}
    ...
    logger.info(f"  TailFlow v5 (1 NFE): {best_crps:.6f}")
    logger.info(f"  Final (100 samples): {results['final']['crps']:.6f}")
    logger.info(f"{'='*60}")
